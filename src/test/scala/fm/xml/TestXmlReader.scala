...
"""
  ...
  private def makeReader(xml: String, root: String = "feed", item: String = "part"): Iterator[SimpleFeedPart] = {
    XmlReaderFactory[SimpleFeedPart](root, item).reader(new StringReader(xml)).toIndexedSeq.iterator
  }
}